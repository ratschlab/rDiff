function [VARIANCE]=get_gene_variance_biol_bin2(CFG,genes,REPLICATES,FRAC_OF_OUTLRS,BIN_LEN,REGION)
...
genes_count_alt=cell(NR_OF_REPLICATES,size(genes,2));
COUNTS=zeros(NR_OF_REPLICATES,size(genes,2));
COUNTS_ALT=zeros(NR_OF_REPLICATES,size(genes,2));
CHK=0;
if 1==0
  for r=1:NR_OF_REPLICATES
	...
	if not(isempty(T{4}))
	  COUNTS_ALT(r,i)=T{4}(1);
	  genes_count_alt{r,i}=T{6};
	  CHK=CHK+1;
	end
      end
    end
  ...
  for r=1:NR_OF_REPLICATES
	...
	if not(isempty(T{7}))
	  COUNTS_ALT(r,i)=T{7}(1);
	  genes_count_alt{r,i}=T{9};
	end
      end
    end
  end
...
clear PROBS
COUNTS_ALT(isnan(COUNTS_ALT))=0;
COUNTS_ALT(isinf(COUNTS_ALT))=0;
COUNTS_ALT=COUNTS_ALT';
...
LENGTH_ALT=0;
for i=POS(1:OUTL_IDX)'
  if and(not(isempty(genes_count_alt{1,i})),not(isempty(genes_count_alt{2,i})))
    %g=[genes_count{1,i};genes_count{2,i}];
    %NON_ZERO=sum(g,[],1)>0;
    %genes_count{1,i}=genes_count{1,i}(NON_ZERO);
    %genes_count{2,i}=genes_count{2,i}(NON_ZERO);
        
    BINS=floor(length(genes_count_alt{1,i})/BIN_LEN);
    T1=[];
    T2=[];
    if BIN_LEN>1
      for j=1:BINS
	T1=[T1 sum(genes_count_alt{1,i}(((j-1)*BIN_LEN+1):(j*BIN_LEN)))];
	T2=[T2 sum(genes_count_alt{2,i}(((j-1)*BIN_LEN+1):(j*BIN_LEN)))];
      end
      genes_count_alt{1,i}=T1;
      genes_count_alt{2,i}=T2;
      LENGTH_ALT=BINS+LENGTH_ALT;
    else
      LENGTH_ALT=length(genes_count_alt{1,i})+LENGTH_ALT;
    end
  end
end
LENGTH_ALT
%MEAN_ALT=zeros(1,LENGTH_ALT);
%VAR_ALT=zeros(1,LENGTH_ALT);

SS=[];
COUNTER=0;
%for i=1:size(genes,2)
for i=POS(1:OUTL_IDX)'
  if and(and(not(isempty(genes_count{1,i})),not(isempty(genes_count{2,i}))),and(not(isempty(genes_count_alt{1,i})),not(isempty(genes_count_alt{2,i}))))
    if sum(genes_count{1,i})+sum(genes_count{2,i})>10
      LENGTH=length(genes_count_alt{1,i});
	...
	continue
      ...
      TEMP_MEAN=mean([genes_count_alt{1,i}/S(1);genes_count_alt{2,i}/S(2)]);
      TEMP_VAR=var([genes_count_alt{1,i}/S(1);genes_count_alt{2,i}/S(2)]);
%      IX=and(and(TEMP_MEAN>140,TEMP_MEAN<150),TEMP_VAR>30000);
%      if sum(IX)>0
%	break
%      end      
...
RR=zeros(1,10000); for i=1:10000,TIDX=and(MEAN>i,MEAN<i*1.1+1);RR(i)= mean(VAR(TIDX));end 
plot(1:10000,RR,'k')
 ylim([0,10000000])
xlim([0,10000])
