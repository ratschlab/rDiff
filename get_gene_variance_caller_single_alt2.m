function []=get_gene_variance_caller_single_alt2(PAR)
...
for i=1:size(genes,2)
  ...
  %GET NON ALT_SPLICED_COVERAGE
  
  %Determine nonspliced regions
  EXON_IDX2=sum(EXONSEQUENCE,1);
  IDX_NON_ALT=zeros(1,size(reads1,2)); 
  STATE=0;  %This means, is alt spliced
  for j=1:length(EXON_IDX2)
    if EXON_IDX2(j)==NR_OF_TRANS     
      IDX_NON_ALT(SPLICINGEVENTS(j):SPLICINGEVENTS(j+1))=1;
    end    
  end

  NON_ALT_READ_IND=sum(reads1(:,IDX_NON_ALT>0),2)>0;
  
  
  
  
  
  IDX_ALT=zeros(1,size(reads1,2)); 
  STATE=0;  %This means, is alt spliced
  for j=1:length(EXON_IDX2)
    if and(EXON_IDX2(j)<NR_OF_TRANS,EXON_IDX2(j)>0)
      IDX_ALT(SPLICINGEVENTS(j):SPLICINGEVENTS(j+1))=1;
    end    
  end
  ALT_READ_IND=sum(reads1(:,IDX_ALT>0),2)>0;
  
  
  READS_ALT=reads1(sum(reads1(:,IDX_ALT>0),2)>0,:);
  
  
  IDX_VECT=zeros(1,size(reads1,2));
  for j=1:length(EXON_IDX2)
    if EXON_IDX2(j)>0
      IDX_VECT(SPLICINGEVENTS(j):SPLICINGEVENTS(j+1))=1;
    end    
  end
  ...
  S1=new_reads1(and(not(ALT_READ_IND),NON_ALT_READ_IND),:);
  
  if or(isempty(S1),sum(IDX_NON_ALT)-COVERED_REGION_LENGTH<1)
    RESULT{4}=[];
    RESULT{5}=0;
    RESULT{6}=[];
  else
    IDX_VECT=and(IDX_NON_ALT,not(NON_UNIQUE_IDX_VECT_STARTS));
  ...
  S1=new_reads1(ALT_READ_IND,:);

  if or(isempty(S1),sum(IDX_ALT>0)-COVERED_REGION_LENGTH<1)
    RESULT{7}=[];
    RESULT{8}=0;
    RESULT{9}=[];
  else
    IDX_VECT=and(IDX_ALT,not(NON_UNIQUE_IDX_VECT_STARTS));
    POSITIONS=max(sum(IDX_VECT>0),1);
    
    COUNT1=S1*(IDX_VECT'>0);
    RESULT{7}=[sum(COUNT1)]/POSITIONS;
    RESULT{8}=POSITIONS;
    RESULT{9}=sum(S1(:,IDX_VECT>0),1);
...
OUT_FILENAME=[CFG.out_base CFG.outfile_prefix '_alt.mat'];
